...
const NEXT_PUBLIC_API_URL = "https://outspoken-pandemic-surfer.ngrok-free.dev";
...
export default function CalibrationPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        // Run summary & ranges in parallel
        const headers = { "ngrok-skip-browser-warning": "true" };
        
        fetch(`${base_api}/api/calibrator/run`, { headers })
          .then((res) => res.json())
          .then((data: SummaryData) => setSummary({ samples_analyzed: data.samples_analyzed || 0 }))
          .catch(console.error);

        fetch(`${base_api}/api/calibration/ranges?machine=${encodeURIComponent(selectedMachine)}&die=${encodeURIComponent(selectedDie)}`, { headers })
          .then((res) => res.json())
          .then((data: Record<string, RangeData>) => setRanges(data))
          .catch(console.error);
  ...
  // Tab Visibility Refresh (Don't update baseline here)
  useEffect(() => {
    const handleVisibility = async () => {
      if (document.visibilityState === "visible") {
